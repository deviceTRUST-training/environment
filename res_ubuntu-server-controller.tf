...
resource "azurerm_virtual_machine" "vm_controller" {
  ...
  primary_network_interface_id        = "${element(azurerm_network_interface.vm_controller_infrastructure.*.id, count.index)}"
  network_interface_ids               = ["${element(azurerm_network_interface.vm_controller_infrastructure.*.id, count.index)}","${element(azurerm_network_interface.vm_controller_main.*.id, count.index)}"]
