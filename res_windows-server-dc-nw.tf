resource "azurerm_public_ip" "vm_dc" {
  ...
  name                 = "${var.azure-environment.prefix}_${var.azure-environment.instance_count}_vm_dc_pip"
...
resource "azurerm_network_interface" "vm_dc" {
  ...
  name                = "${var.azure-environment.prefix}_${var.azure-environment.instance_count}_vm_dc_nic"
  location            = "${element(azurerm_resource_group.main.*.location, count.index)}"
  resource_group_name = "${element(azurerm_resource_group.main.*.name, count.index)}"
  tags                = "${var.tags}"
  ...
  ip_configuration {
    ...
    private_ip_address            = "${var.azure-environment.ip_prefix}${var.azure-environment.instance_count}.${var.vm.ip_dc}"
